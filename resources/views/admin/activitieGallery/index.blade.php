...
<div class="container font">
    <div class="breadcrumbs ace-save-state" id="breadcrumbs">
        <ul class="breadcrumb">
            <li class="active">Activitie Gallery Image Information</li>
        </ul><!-- /.breadcrumb -->
    </div>
    @if ($errors->any())
    ...
    <div class="page-content font">
        ...
        <div class="row">
            <div class="col-xs-12">
               ...
               @endif
                <!-- PAGE CONTENT BEGINS -->
                <form class="form-horizontal" role="form" method="POST" action="{{route('activitiegallerys.store')}}"
    ...
    <?php
    $activitieProgramsCount = $activitieGalleys->count();    
    ?>
    <h3 class="header smaller lighter blue d-inline">All Activitie Gallery Image ({{ $activitieProgramsCount }})</h3> 
                ...
                <table id="example" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="dynamic-table_info">
                    ...
                    <tbody>
                        @php
                            $sn = 1;
                        @endphp
                        @foreach ($activitieGalleys as $activitie)
                            <tr role="row" class="{{ $loop->odd ? 'odd' : 'even' }}">
                                <td class="center">{{ $sn++ }}</td>
                                <td class="text-center">
                                    <img width="50" class="mx-auto d-block" src="{{ asset('images/activitieGallery/' . $activitie->image) }}" alt="">
                                </td>
                                
                                <td class="btn-group" style="display: flex; justify-content: center">
                                    <form action="{{ route('activitiegallerys.destroy',$activitie->id) }}" method="POST">
                                        @csrf
                                        {{-- @method('DELETE') --}}
